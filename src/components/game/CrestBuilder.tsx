...
import { Shuffle, FlipHorizontal, Save, Crown, Shield, Palette, Settings2, Layers, RotateCcw, Sparkles } from 'lucide-react';
...
const HERALDIC_PAIRS: Array<[string, string, string]> = [
  ...
  ['#4A148C', '#FFFFFF', '#FFD700'], // Roxo/Branco/Dourado
...
/* Layout patterns shown in the "Layout" tab — heraldic divisions first */
...
/* ── Section divider with title (referência: ──── ANIMAIS ────) ── */
...
/* ── Mono catalog card (white square + black silhouette, heraldic style) ── */
    ...
    <div className="grid gap-4 md:grid-cols-[260px_1fr]">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid grid-cols-5 h-auto w-full">
          <TabsTrigger value="shape" className="flex-col gap-0.5 py-1.5 text-[10px]">
            <Shield className="h-3.5 w-3.5" />Forma
          </TabsTrigger>
          <TabsTrigger value="layout" className="flex-col gap-0.5 py-1.5 text-[10px]">
            <Layers className="h-3.5 w-3.5" />Divisão
          </TabsTrigger>
          <TabsTrigger value="symbol" className="flex-col gap-0.5 py-1.5 text-[10px]">
            <Sparkles className="h-3.5 w-3.5" />Símbolo
          </TabsTrigger>
                ...
                >
                  <div className="flex flex-col items-center gap-1">
                    <ShieldCrest
                      ...
                      size={52}
                    />
                    <span className="text-[9px] font-medium text-zinc-700 leading-tight">
                      {SHAPE_LABELS[shape]}
                    </span>
                  </div>
                </CatalogCard>
        ...
        <TabsContent value="layout" className="mt-3">
          <ScrollArea className="h-[340px] pr-2">
            ...
            {/* Outros padrões em seção secundária */}
        ...
        {/* ─── SYMBOL TAB (3 sections in one scrollable view, like reference) ─── */}
          ...
          <ScrollArea className="h-[320px] pr-2">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 mt-2 mb-4">
              ...
              {ANIMAL_ICONS.map(ic => (
                ...
                >
                  {/* mono silhouette: black on white */}
                  <ShieldCrest
                    primaryColor="#FFFFFF"
                    secondaryColor="#FFFFFF"
                    detailColor="#0A0A0A"
