...
import { LayoutDashboard, Newspaper, Users, Target, Trophy, ShoppingBag } from 'lucide-react';
  ...
  return (
    <div className="flex-1 flex flex-col gap-1">
      <TabsList className="grid grid-cols-6 h-auto gap-0.5 bg-card/60 backdrop-blur-sm p-1 border border-border/20 rounded-xl overflow-hidden">
        ...
        <TabsTrigger value="shop" className="nav-tab flex flex-col items-center gap-0.5 py-1.5 sm:py-2.5">
          <ShoppingBag className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-emerald-400" />
          <span className="text-[9px] sm:text-xs leading-tight font-medium">Loja</span>
        </TabsTrigger>
      </TabsList>
    </div>
  );
