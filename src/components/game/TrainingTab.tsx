/**
 * TrainingTab V3 — Sistema de Progresso (%)
 * - Header: Card grande do CT (nível, eficiência %, custo upgrade)
 * - Cards de jogadores: barra 0-100%, status, dropdown grupo/específico, intensidade
 * - Ícones ❓ abrem TrainingHelpPopup em tópico específico
 */
...
import { Dumbbell, HelpCircle, TrendingUp, AlertTriangle, Flame, Scale, Turtle, X, Zap } from 'lucide-react';
...
function PlayerTrainingCard({
  player, focus, intensity, gain, status, onChangeFocus, onChangeIntensity, onHelp,
}: {
  player: Player;
  ...
  return (
    <div className={`rounded-xl border p-3 sm:p-4 space-y-3 ${player.injury ? 'border-red-500/30 bg-red-500/5' : 'border-border/30 bg-card/40'}`}>
      {/* Header: name + age + OVR + status */}
      <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
        ...
        {player.injury && <Badge variant="destructive" className="text-[9px] h-5">🏥 Lesão {player.injury.weeksRemaining}sem</Badge>}
      </div>
          ...
          {filtered.sort((a, b) => b.overall - a.overall).map(p => {
            ...
            return (
              <PlayerTrainingCard
                ...
                gain={gain}
                status={status}
                onChangeFocus={(f) => onSetTrainingFocus(p.id, f)}
                onChangeIntensity={(i) => onSetTrainingIntensity?.(p.id, i)}
                onHelp={openHelp}
              />
            );
          })}
