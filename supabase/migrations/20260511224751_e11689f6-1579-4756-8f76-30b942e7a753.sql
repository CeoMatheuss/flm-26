...
BEGIN
  ...
  IF _shared_match_id LIKE 'friendly-%' AND to_regclass('public.friendly_invites') IS NOT NULL THEN
    EXECUTE '
      SELECT EXISTS (
  ...
  IF to_regclass('public.international_matches') IS NOT NULL THEN
    EXECUTE '
      SELECT EXISTS (
        SELECT 1 FROM public.international_matches im
        WHERE $1 = im.id::text
          AND (im.home_user_id = $2 OR im.away_user_id = $2)
      )'
    INTO _ok USING _shared_match_id, _user_id;
    IF _ok THEN RETURN true; END IF;
  END IF;
...
BEGIN
  ...
  IF _match_id LIKE 'friendly-%' AND to_regclass('public.friendly_invites') IS NOT NULL THEN
    ...
    EXCEPTION WHEN others THEN
      RETURN NULL;
    END;
    EXECUTE 'SELECT sender_id FROM public.friendly_invites WHERE id = $1'
  ...
  IF to_regclass('public.international_matches') IS NOT NULL THEN
    EXECUTE 'SELECT home_user_id FROM public.international_matches WHERE id = $1'
      INTO _home USING _uuid;
    IF _home IS NOT NULL THEN RETURN _home; END IF;
  END IF;
