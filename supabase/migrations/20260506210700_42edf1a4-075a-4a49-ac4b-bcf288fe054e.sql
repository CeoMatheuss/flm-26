...
-- 2. ENSURE EXACTLY 16 TEAMS FOR THE MAIN LEAGUE (BR Division 1)
DO $$
DECLARE
    br_league_id UUID;
    team_count INTEGER;
...
BEGIN
    ...
    IF br_league_id IS NULL THEN
        INSERT INTO public.world_leagues (league_name, country, flag_emoji, division, total_matchdays, kickoff_hour, kickoff_minute, status, season)
        VALUES ('Brasileirão Série A', 'BR', '🇧🇷', 1, 30, 19, 30, 'in_progress', 1)
    ...
    -- If more than 16, remove extra bots
...
-- 3. FIXTURE GENERATION (Circle Method)
DO $$
DECLARE
    br_league_id UUID;
    l_season INTEGER;
...
BEGIN
    ...
    FOR r IN 1..num_rounds LOOP
        ...
        FOR idx IN 0..(num_teams/2 - 1) LOOP
            ...
            -- Update standings for simulated games
            IF r < 5 THEN
                UPDATE public.world_league_teams 
                SET 
                    played = played + 1,
